...
// SM120 Block-Scaled Grouped GEMM Launcher
// This launcher supports block-scaled operations on SM120/SM121 (Blackwell) architecture:
//
// 1. MXFP4 (IsMXFP4=true): Mixed-input FP8×FP4
//    - FP8 activations × FP4 weights
//    - BF16/FP16 activations are pre-quantized to FP8 before the kernel
//    - Uses mx_float8_t + mx_float4_t (MX format)
//
// 2. NVFP4 (IsMXFP4=false): Same-type FP4×FP4
//    - FP4 activations × FP4 weights (native)
//    - Uses nv_float4_t (NV format)
//
// Key features:
...
// For MXFP4: BF16/FP16 activations must be pre-quantized to FP8.
// For NVFP4: Native FP4 activations are used directly.
//
...
// SM120 Mixed-Input Launcher Entry Point
// Simplified signature matching the generic TMA WS launcher pattern.
// Note: Unlike the generic launcher, this does NOT need EpilogueSchedule since
// SM120 block-scaled uses a fixed schedule (KernelPtrArrayTmaWarpSpecializedCooperativeBlockScaledSm120).
...
void sm120_mixed_input_moe_gemm_kernelLauncher(
    TmaWarpSpecializedGroupedGemmInput hopper_inputs, int num_experts, int sm_count_,
